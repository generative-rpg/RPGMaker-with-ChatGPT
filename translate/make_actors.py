...
import numpy as np
from maker import BASE_PATH, GAME_NAME, init_data
...
def make_actors():
    ...
    for num, actor_info in enumerate(members):
        ...
        for skill_name in class_skills:
            ...
            for i, param_name in enumerate(["hp", "mp", "atk", "def", "mat", "mdf", "agi", "luk"]):
                param_100 = (
                    int(actor_info[param_name] * 8 * random.randrange(80, 120) / 100) if param_name != "mp" else 0
                )
            ...
            data["actors"].append(
                {
                    ...
                    "classId": class_id,
                    "equips": [0, 0, 0, 0, 0],
                    "faceIndex": (num % 8),  # 顔番号仮決め(0, 1, 2, ..., 7)
                    "faceName": f"grid{int(num/8) + 1}",  # 顔画像仮決め(grid1, grid2, ...)
                    "traits": [],
                    "initialLevel": 1,
                    "maxLevel": 99,
    ...
    with open(f"{BASE_PATH}/{GAME_NAME}/data/Classes.json", mode="w", encoding="utf8") as f:
        json.dump(data["classes"], f, indent=2, ensure_ascii=False)
    with open(f"{BASE_PATH}/{GAME_NAME}/data/Actors.json", mode="w", encoding="utf8") as f:
