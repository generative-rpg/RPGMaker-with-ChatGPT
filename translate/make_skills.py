...
from maker import BASE_PATH, GAME_NAME, get_animation_id, get_element_id, init_data
...
def get_effect_dict(data, effect_name, rate=0):
    ...
    try:
        ...
        print(f"not found state {effect_name}")
        raise e
    return [
...
def make_skills(skill_key):
    data, gpt = init_data()
    skills = gpt[skill_key]
    ...
    for skill_info in skills:
        ...
        for effect_name in skill_info["target_add_state"]:
            effects.extend(get_effect_dict(data, effect_name, skill_info["target_add_state_rate"]))
        # name
